...
class ALNS:
    ...
    def run(self, seed_init=0):
        ...
        # Temperatura inicial (se não fornecida): baseada na variância de movimentos aleatórios
        if self.T is None:
            samples = []
            for _ in range(20):
                tmp = current.clone()
                ...
                if len(self.data.tarefas) >= 2:
                    ...
                    # Ensure removed tasks are inserted back, or the loop will break early
                    if removed:
                        insert_greedy(tmp, removed, self.rng, noise=self.noise)
                evaluate_solution(tmp) # Ensure C is calculated for sample
                samples.append(tmp.C)
            # Calculate standard deviation if samples are valid, otherwise default T
            if len(samples) > 1 and not all(c == samples[0] for c in samples):
                ...
                self.T = 10.0 # Default if stdev cannot be computed or all samples are same

        visited = set()  # hash simples por tupla ordenada (pouco rigoroso, mas útil)
        def signature(sol):
            # assinatura pela estação: (w, sorted(tasks))
            sig = []
        ...
        while iter_count < self.max_iter:
            ...
            # Passo workers (opcional, usado com probabilidade moderada)
            # Only attempt worker ops if there are at least 2 stations/workers
            if self.data.m >= 2 and self.rng.random() < 0.3:
                w_op(candidate, self.rng)

            # Passo remove/insert
            q_min = max(1, int(self.remove_frac[0] * self.data.n)) # Ensure at least 1 task is removed if possible
            q_max = min(self.data.n, int(self.remove_frac[1] * self.data.n)) # Max tasks to remove is n
            q = self.rng.randint(q_min, max(q_min, q_max)) # Ensure q_max is not less than q_min

            removed_tasks = r_op(candidate, q, self.rng)

            # Attempt to insert removed tasks back
            ok = i_op(candidate, removed_tasks, self.rng, noise=self.noise)
            if not ok: # If insertion fails
                # Try greedy as fallback, but only if regret failed to place all tasks
                ok = insert_greedy(candidate, removed_tasks, self.rng, noise=self.noise)
                if not ok: # If even greedy fails to place all tasks, discard iteration
                    iter_count += 1
                    segment_counter += 1
                    self.T *= self.cooling
                    if segment_counter >= self.segment_length:
                        self.update_weights()
                        segment_counter = 0
                    continue # Skip to next ALNS iteration

            # Avaliação e aceitação
            evaluate_solution(candidate) # Ensure candidate.C is up-to-date after insertions
            ...
            # Atualizar registros e pesos (pontuações)
            if is_new_best:
